...
if __name__ == "__main__":
    ...
    inputProcess = multiprocessing.Process(
        target=inputProcessLoop,
        args=(
            queueToInputProcess,
            queueFromInputProcess,
        ),
    )
    inputProcess.start()
    ...
    outputProcess = multiprocessing.Process(
        target=outputProcessLoop, args=(queueToOutputProcess,)
    )
    outputProcess.start()
    ...
    while not done:
        if not queueFromInputProcess.empty():
            ...
            if (
                from_queue[0] == "\x1b"
            ):  # THIS CODE ENDS EVERYTHING NAND CLOSES OUT ALL THE PROCESSES
                queueToInputProcess.put(
                    "quit"
                )  # put quit on the queue's to signal done
                queueToOutputProcess.put(
                    "quit"
                )  # put quit on the queues to signal done
                ...
                from_queue[0] == "i"
            ):  # from queue is input then display the input or do some random shit I don't know
                screen = pygame.display.set_mode((500, 500))
                screen.fill((0, 0, 0))
                pygame.display.flip()
        ...
        if updateDisplay:
            ...
            thisRender = feedbackFont.render(
                toDisplay, True, (255, 255, 255)
            )  # 	TODO: i don't know what this does
            x = (
                screen.get_width() / 2 - thisRender.get_width() / 2
            )  #  TODO:I also don't kow what this does
            ...
            queueToOutputProcess.put(
                flipLatency
            )  # TODO: idk what this does maybe metrics?
